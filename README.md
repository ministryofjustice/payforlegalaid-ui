...
By default, authentication is disabled locally. To set it you need to update the .env to have `AUTH_ENABLED = true`, and set the `CLOUD_INSTANCE`, `CLIENT_SECRET`, `CLIENT_ID` and `TENANT_ID` to match the development Entra ID application. You should only test against the dev instance of Entra ID from your local machine, and not the live instance.
