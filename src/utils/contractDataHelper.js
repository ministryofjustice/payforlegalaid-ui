...
export function getSampleForPath(endpoint, method, statusCode) {
  ...
  if (!endpointPaths) {
    throw new Error(
      `No definition found for endpoint ${endpoint} and method ${method}`,
    );
  }
  ...
  if (!responseDefinition) {
    throw new Error(
      `No response defined for status code ${statusCode} at ${endpoint}`,
    );
  }
  ...
  if (!content) {
    throw new Error(
      `No application/json content found for ${endpoint} ${method} ${statusCode}`,
    );
  }
